...
class MainBottomAppBar extends StatelessWidget implements PreferredSizeWidget {
  const MainBottomAppBar(
      {super.key, this.title, this.leading, this.titleWidget, this.heading});
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<HomeAppBarCarouselController>(
      builder: (controller) {
        return heading != null
            ? CarousalTextButton(text: heading!)
            : CarouselSlider(
                ...
                items: [
                  CarousalTextButton(
                    text: 'Favourites',
                    onPressed: () =>
                        homeScreenPageController.animateAndChangePage(0),
                  ),
                  CarousalTextButton(
                    text: 'Playlists',
                    onPressed: () =>
                        homeScreenPageController.animateAndChangePage(1),
                  ),
                  CarousalTextButton(
                    text: 'Tracks',
                    onPressed: () =>
                        homeScreenPageController.animateAndChangePage(2),
                  ),
                  CarousalTextButton(
                    text: 'Albums',
                    onPressed: () =>
                        homeScreenPageController.animateAndChangePage(3),
                  ),
                  CarousalTextButton(
                    text: 'Artists',
                    onPressed: () =>
                        homeScreenPageController.animateAndChangePage(4),
                  ),
                  CarousalTextButton(
                    text: 'Folders',
                    onPressed: () =>
                        homeScreenPageController.animateAndChangePage(5),
                  ),
                ],
